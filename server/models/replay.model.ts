...
class ReplayModel {
	...
	async create(u: ReplayTypes): Promise<ReplayTypes> {
		try {
			...
			const sql =
				'INSERT INTO replay (comment_id, assistant_id, student_id, text, file_url, file_type, shown) VALUES($1, $2, $3, $4, $5, $6, $7) returning *'
	...
	async update(u: ReplayTypes): Promise<ReplayTypes> {
		try {
			const connect = await pool.connect()
			const sql =
				'UPDATE replay SET text=($1), file_url=($2), file_type=($3), shown=($4)  WHERE id=($5) returning *'
			const result = await connect.query(sql, [
				u.text,
				u.file_url,
				u.file_type,
